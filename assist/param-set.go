...
import (
	...
	"github.com/snivilised/mamba/internal/third/lo"
...
func NewParamSet[N any](command *cobra.Command, overrides ...ShortFlagOverride) (ps *ParamSet[N]) {
	...
	if reflect.TypeOf(*ps.Native).Kind() != reflect.Struct {
		...
		panic(locale.NewParamSetObjectMustBeStructNativeError(command.Name(), typeName))
	}

	if len(overrides) > 0 {
		v := reflect.ValueOf(ps.Native).Elem()
		f := v.FieldByName("Overrides")
		if f.IsValid() && f.CanSet() {
			f.Set(reflect.ValueOf(overrides))
		}
	}
